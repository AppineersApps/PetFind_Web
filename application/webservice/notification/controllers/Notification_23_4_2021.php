...
class Notification extends Cit_Controller
{
    ...
    {
        if(false == empty($request_arr['page_name']) && 'found_notify' == $request_arr['page_name'])
        {
        $valid_arr = array(
            "user_id" => array(
                array(
    ...
    public function send_message($request_arr = array(), $inner_api = FALSE)
    {
        ...
        try
        {
            ...
            if (false == empty($input_params["checkPostStatus"][0]['missing_pet_id']) && isset($block_status['checkBlockStatus']['missing_pet_id']))
            {
    ...
    public function get_user_details_for_send_notifi($input_params = array())
    {
        ...
        try
        {
           ...
           $tagged_user_result = $this->notification_model->get_tagged_user_details_for_send_notifi($user_id, $missing_pet_id);
            ...
            if(isset($input_params["page_code"]) && 'found_notify'==$input_params["page_code"])
            {
    ...
    public function post_notification($input_params = array())
    {
        ...
        try
        {
            ...
            if(isset($input_params["page_code"]) && 'found_notify'==$input_params["page_code"])
            {
                $this->post_result = $this->missing_pet_model->update_missing_pet($params_arr,$where_arr);
            }
            
            // print_r($this->post_result);exit;
        }
        catch(Exception $e)
        {
            $success = 0;
...
// print_r($input_params);exit;
    ...
    public function messages_finish_failure($input_params = array())
    {
       ...
       if($input_params["checkPostStatus"]["checkpoststatus"][0]['message'])
       {
        $message=$input_params["checkPostStatus"]["checkpoststatus"][0]['message'];
           $setting_fields = array(
            "success" => "0",
            "message" => $message,
          );
