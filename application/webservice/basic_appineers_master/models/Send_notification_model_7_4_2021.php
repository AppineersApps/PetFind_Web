...
$result_arr = array_map(function (array $arr) {
    ...
    public function get_user_details_for_send_notifi_area($input_params)
    {
        ...
        try
        {
            ...
            {
                $tagged_user_id=$input_params['receiver_id'];
                $strWhere =" u.tAddress = '".$input_params['last_seen_street']."'  AND u.vCity ='".$input_params['last_seen_city']."' AND u.iUserId != '".$tagged_user_id."'";
            }
            else
            {
            ...
            distinct(u.iUserId) AS receiver_id,
             ...
             CONCAT(u.vFirstName,\" \",u.vLastName) AS u_name
             FROM users AS u            
             INNER JOIN missing_pets AS misp ON (misp.iUserId = u.iUserId)             
             WHERE $strWhere";
            $result_obj = $this->db->query($strSql);
