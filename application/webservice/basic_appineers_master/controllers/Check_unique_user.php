...
class Check_unique_user extends Cit_Controller
{
     ...
     * @return array $valid_res returns output response of API.
     */
    public function rules_email_feilds($request_arr = array())
    {
        $valid_arr = array(
     ...
     * @return array $valid_res returns output response of API.
     */
    public function rules_phone_feilds($request_arr = array())
    {
        $valid_arr = array(
     ...
     * @return array $valid_res returns output response of API.
     */
    public function rules_types_feilds($request_arr = array())
    {
        //    $res= $this->form_validation->set_rules('type', 'type', 'in_list[email,phone]');

        $valid_arr = array(
            "type" => array(
                array(
                    "rules" => 'trim|required|in_list[phone,email]',
                    "value" => TRUE,
                    "message" => "valid_type_required",
                )
            )
        );
        $valid_res = $this->wsresponse->validateInputParams($valid_arr, $request_arr, "check_unique_user");
    ...
    public function start_check_unique_user($request_arr = array(), $inner_api = FALSE)
    {
        try {
            ...
            if ($validation_res["success"] == "-5") {
                ...
                if ($request_arr['type'] == 'email') {
                    $validation_res = $this->rules_email_feilds($request_arr);
                    if ($validation_res["success"] == "-5") {
                        if ($inner_api === TRUE) {
                            return $validation_res;
                        } else {
                ...
                } else if ($request_arr['type'] == 'phone') {
                    $validation_res = $this->rules_phone_feilds($request_arr);
                    if ($validation_res["success"] == "-5") {
                        if ($inner_api === TRUE) {
                            return $validation_res;
                        } else {
                    ...
                    $validation_res = $this->rules_types_feilds($request_arr);
