...
<%include file="abusive_reports_for_post_add_strip.tpl"%>
                        ...
                        <div class="content <%$rl_theme_arr['frm_stand_label_align']%>">
                            <div class="form-row row-fluid " id="cc_sh_arfp_reported_by">
                                ...
                                <div class="form-row row-fluid " id="cc_sh_arfp_post_id">
                                    <!-- <label class="form-label span3 ">
                                        <%$form_config['arfp_missing_pet_id']['label_lang']%>
                                    </label> --> 
                                    <div class="form-right-div  <%if $mode eq 'Update'%>frm-elements-div<%/if%> ">
                                        <%if $mode eq "Update"%>
                                            <input type="hidden" class="ignore-valid" name="arfp_missing_pet_id" id="arfp_missing_pet_id" value="<%$data['arfp_missing_pet_id']|@htmlentities%>" />
                                            <!-- <span class="frm-data-label">
                                                <strong>
                                                    <%if $data['arfp_missing_pet_id'] neq ""%>
                                                        <%$data['arfp_missing_pet_id']%>
                                                    <%else%>
                                                    <%/if%>
                                                </strong></span> -->
                                            <%else%>
                                                <input type="text" placeholder="" value="<%$data['arfp_missing_pet_id']|@htmlentities%>" name="arfp_missing_pet_id" id="arfp_abusive_reports_for_missing_pet_id" title="<%$this->lang->line('ABUSIVE_REPORTS_FOR_REVIEW_REVIEW')%>"  data-ctrl-type='textbox'  class='frm-size-medium'  />
                                            <%/if%>
                                        </div>
                                        <div class="error-msg-form "><label class='error' id='arfp_abusive_reports_for_missing_pet_idErr'></label></div>
                                    </div>
                                    <%assign var="child_module_name" value="user_missing_pet_images"%>
                                                                    ...
                                                                    <div class="form-row row-fluid form-inline-child" id="child_module_<%$child_module_name%>">
                                                                        ...
                                                                        <%section name=i loop=$child_ord%>
                                                                            ...
                                                                            <%assign var="child_id" value=$child_data[i]['iUsermissing_petImageId']%>
                                                                            <%assign var="enc_child_id" value=$this->general->getAdminEncodeURL($child_id)%>
                                                                            <%assign var="child_id_temp" value=[$child_data[i]['uri_missing_pet_image']]%>
                                                                            <%assign var="child_merge_data" value=$child_merge_data|@array_merge:$child_id_temp%>
                                                                            <input type="hidden" name="child[user_missing_pet_images][id][<%$row_index%>]" id="child_user_missing_pet_images_id_<%$row_index%>" value="<%$child_id%>" />
                                                                            <input type="hidden" name="child[user_missing_pet_images][enc_id][<%$row_index%>]" id="child_user_missing_pet_images_enc_id_<%$row_index%>" value="<%$enc_child_id%>" />
                                                                            <input type="hidden" name="child[user_missing_pet_images][uri_user_missing_pet_id][<%$row_index%>]" id="child_user_missing_pet_images_uri_user_missing_pet_id_<%$row_index%>" value="<%$child_data[i]['uri_user_missing_pet_id']%>"  class='ignore-valid ' />
                                                                            <input type="hidden" name="child[user_missing_pet_images][uri_added_at][<%$row_index%>]" id="child_user_missing_pet_images_uri_added_at_<%$row_index%>" value="<%$child_data[i]['uri_added_at']%>"  class='ignore-valid '  aria-date-format='yy-mm-dd'  aria-format-type='date' />
                                                                            <input type="hidden" name="child[user_missing_pet_images][uri_status][<%$row_index%>]" id="child_user_missing_pet_images_uri_status_<%$row_index%>" value="<%$child_data[i]['uri_status']%>"  class='ignore-valid ' />
                                                                        <%/section%>
                                                                        ...
                                                                        <div class="form-right-div form-inline-child <%if $recMode eq "Update"%>frm-elements-div<%/if%>"  id="child_module_rel_<%$child_module_name%>">
                                                                            <div id="upload_multi_file_user_missing_pet_images" class="upload-multi-file frm-size-medium clear">
                                                                                <%assign var="is_images_exists" value=0%>
                                                                                <%section name=j loop=$child_cnt%>
                                                                                    <%assign var="row_index" value=$smarty.section.j.index%>
                                                                                    <%if $child_img_html[$row_index]["uri_missing_pet_image"] neq ""%>
                                                                                        <%assign var="is_images_exists" value=1%>
                                                                                        <div class="row-upload-file" id="upload_row_user_missing_pet_images_<%$row_index%>">
                                                                                            <input type="hidden" value="<%$child_data[j]['uri_missing_pet_image']%>" name="child[user_missing_pet_images][old_uri_missing_pet_image][<%$row_index%>]" id="child_user_missing_pet_images_old_uri_missing_pet_image_<%$row_index%>" />
                                                                                            <input type="hidden" value="<%$child_data[j]['uri_missing_pet_image']%>" name="child[user_missing_pet_images][uri_missing_pet_image][<%$row_index%>]" id="child_user_missing_pet_images_uri_missing_pet_image_<%$row_index%>" class="ignore-valid" aria-extensions="gif,png,jpg,jpeg,jpe,bmp,ico" aria-valid-size="<%$this->lang->line('GENERIC_LESS_THAN')%> (<) 100 MB"/>
                                                                                            <div class="">
                                                                                                <%$child_img_html[$row_index]["uri_missing_pet_image"]%>
                                                                                            </div>
                                                                                        </div>
                                                                                    <%/if%>
                                                                                <%/section%>
                                                                                <%if $is_images_exists eq 0%>
                                                                                    <input type="hidden" value="" name="child[user_missing_pet_images][uri_missing_pet_image][0]" id="child_user_missing_pet_images_uri_missing_pet_image_0" class="_upload_req_file"/>
                                                                                <%/if%>
                                                                            </div>
                                                                            <div class="clear"></div>
                                                                        </div>
                                    ...
                                    <div class="form-right-div  <%if $mode eq 'Update'%>frm-elements-div<%/if%> ">
                                       <input type="text" placeholder="" value="<%$data['p_post_title']|@htmlentities%>" name="p_post_title" id="p_post_title" title="<%$this->lang->line('ABUSIVE_REPORTS_FOR_missing_petS_MESSAGE')%>"  data-ctrl-type='textbox'  class='frm-size-medium'  />
                                ...
                                <div class="frm-bot-btn <%$rl_theme_arr['frm_stand_action_bar']%> <%$rl_theme_arr['frm_stand_action_btn']%> popup-footer">
                                    ...
                                    <%/if%>
                                    <%include file="abusive_reports_for_missing_pets_add_buttons.tpl"%>
                                </div>
                            </div>
                            <div class="clear"></div>
                        </form>
