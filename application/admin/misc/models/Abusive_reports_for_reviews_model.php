...
class Abusive_reports_for_reviews_model extends CI_Model 
{
    ...
    public function __construct() 
    {
        ...
        $this->grid_fields = array("u_first_name", "owner_first_name","p_post_title", "arfp_message", "res_reason","arfp_added_at", "status","sys_custom_field_1","ron_users_id",
            "rb_users_id");
    ...
    public function getData($extra_cond = "", $fields = "", $order_by = "", $group_by = "", $limit = "", $join = "No", $having_cond = '', $list = FALSE)
    {
        ...
        $this->db->select("arfp.iMissingPetId AS arfp_review_id");
        $this->db->select("u.iUserId AS ron_users_id");
            $this->db->select("owner.iUserId AS rb_users_id");
        
        } else {
            $this->db->select("arfp.iAbusiveReportsForMissingPetId AS iAbusiveReportsForReviewId");
