...
/**
 * Description of Abusive Reports For Missing post Controller
 *
 * @category admin
 *
 * @package misc
 ...
 *
 * @module Abusive Reports For posts
 *
 * @class Abusive_reports_for_post.php
 *
 * @path application\admin\misc\controllers\Abusive_reports_for_post.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Abusive_reports_for_post extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('abusive_reports_for_post_model');
        $this->_request_params();
        $this->folder_name = "misc";
        $this->module_name = "abusive_reports_for_post";
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "abusive_reports_for_post_list",
                    "abusive_reports_for_post_view",
                    "abusive_reports_for_post_add",
                    "abusive_reports_for_post_update",
                    "abusive_reports_for_post_delete",
                    "abusive_reports_for_post_export",
                    "abusive_reports_for_post_print",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("abusive_reports_for_post", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "abusive_reports_for_post");
            $status_array = $status_label = array();
            $list_config = $this->abusive_reports_for_post_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "abusive_reports_for_post");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->abusive_reports_for_post_model->default_filters,
            );
            ...
            {
                $this->loadView("abusive_reports_for_post_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->abusive_reports_for_post_model->getListConfiguration();
        $form_config = $this->abusive_reports_for_post_model->getFormConfiguration();
        $extra_cond = $this->abusive_reports_for_post_model->extra_cond;
        $groupby_cond = $this->abusive_reports_for_post_model->groupby_cond;
        $having_cond = $this->abusive_reports_for_post_model->having_cond;
        $orderby_cond = $this->abusive_reports_for_post_model->orderby_cond;
        ...
        $data_recs = $this->abusive_reports_for_post_model->getListingData($data_config);
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_ABUSIVE_REPORTS_FOR_post_DATA_FOUND_C46_C46_C33');

        $data_recs = $this->abusive_reports_for_post_model->processCustomLinks($data_recs, $list_config);
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("abusive_reports_for_post_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("abusive_reports_for_post", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->abusive_reports_for_post_model->getListConfiguration();
        $form_config = $this->abusive_reports_for_post_model->getFormConfiguration();
        $table_name = $this->abusive_reports_for_post_model->table_name;
        $table_alias = $this->abusive_reports_for_post_modeltable_alias;
        $primary_key = $this->abusive_reports_for_post_model->primary_key;
        $extra_cond = $this->abusive_reports_for_post_model->extra_cond;
        $groupby_cond = $this->abusive_reports_for_post_model->groupby_cond;
        $having_cond = $this->abusive_reports_for_post_model->having_cond;
        $orderby_cond = $this->abusive_reports_for_post_model->orderby_cond;
        ...
        $db_recs = $this->abusive_reports_for_post_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('ABUSIVE_REPORTS_FOR_post_ABUSIVE_REPORTS_FOR_post');
        $misc_info['filename'] = "abusive_reports_for_post_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("abusive_reports_for_post_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("abusive_reports_for_post", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("abusive_reports_for_post", $access_list, TRUE, TRUE);
                }
                if (!$add_access)
                {
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "abusive_reports_for_post"), $this->session->userdata('iAdminId'));
                ...
                $parent_join_arr = array(
                    "joins" => array(
                        array(
                            ...
                            "rel_table_name" => "abusive_reports_for_post",
                ...
                $data_arr = $this->abusive_reports_for_post_model->getData(intval($id),"","","","",$parent_join_arr);
           ...
           // $child_config["callback"] = "controller::getReviewImages";
            ...
            $child_assoc_access["user_review_images"] = array(
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "abusive_reports_for_post", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "abusive_reports_for_post"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "abusive_reports_for_post");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->abusive_reports_for_post_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->abusive_reports_for_post_model, $phpfunc))
                            {
                                $tmpdata = $this->abusive_reports_for_post_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->abusive_reports_for_post_model, $fd_callback))
                                {
                                    $fd_status = $this->abusive_reports_for_post_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("abusive_reports_for_post_add");
                    }
                    else
                    {
                        $this->loadView("abusive_reports_for_post_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("abusive_reports_for_post_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("abusive_reports_for_post_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("abusive_reports_for_post", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            if ($mode == 'Add')
            {
                ...
                $switch_combo = $this->abusive_reports_for_post_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "abusive_reports_for_post", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update')
            {
                 $this->load->model("basic_appineers_master/missing_pet_management_model"); 
                 if($data["eStatus"] == "Rejected")
                 {
                    $textdata['eStatus'] = 'Inactive';
                    $res = $this->missing_pet_management_model->update($textdata, intval($arfp_missing_pet_id));
                 }
                 else
                 {
                   $res = $this->missing_pet_management_model->update($textdata, intval($arfp_missing_pet_id)); 
                ...
                if (intval($res) > 0)
                {
                     $res = $this->abusive_reports_for_post_model->update($data, intval($id));
                    $save_data_arr["iAbusiveReportsForMissingPetId"] = $data["iAbusiveReportsForMissingPetId"] = $id;
                ...
                $switch_combo = $this->abusive_reports_for_post_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "abusive_reports_for_post", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
        ...
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")))
        {
            ...
            $config_arr['list_config'] = $this->abusive_reports_for_post_model->getListConfiguration();
            $config_arr['form_config'] = $this->abusive_reports_for_post_model->getFormConfiguration();
            $config_arr['table_name'] = $this->abusive_reports_for_post_model->table_name;
            $config_arr['table_alias'] = $this->abusive_reports_for_post_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->abusive_reports_for_post_model->table_alias.".".$this->abusive_reports_for_post_model->primary_key;
        }
        else
        {
            $primary_field = $this->abusive_reports_for_post_model->primary_key;
        }
        ...
        try
        {
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("abusive_reports_for_post_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("abusive_reports_for_post", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    $success = $this->abusive_reports_for_post_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                    }
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("abusive_reports_for_post_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("abusive_reports_for_post", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->abusive_reports_for_post_model->getListConfiguration($post_name);
                    $form_config = $this->abusive_reports_for_post_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $success = $this->abusive_reports_for_post_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->abusive_reports_for_post_model->table_alias.".";
                    }
                    else
                    {
                    ...
                    $success = $this->abusive_reports_for_post_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->abusive_reports_for_post_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->abusive_reports_for_post_model, $phpfunc))
                    {
                        $data_arr = $this->abusive_reports_for_post_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->abusive_reports_for_post_model->switchto_fields;
        $extra_cond = $this->abusive_reports_for_post_model->extra_cond;
        ...
        $switch_arr = $this->abusive_reports_for_post_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->abusive_reports_for_post_model->getListConfiguration($alias_name);
        $source_field = $config_arr['source_field'];
        $combo_config = $this->dropdown_arr[$source_field];
        $data_arr = array();
        if ($mode == "Update")
        {
            $data_arr = $this->abusive_reports_for_post_model->getData(intval($id));
        }
